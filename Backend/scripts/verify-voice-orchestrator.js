...
stt.publish({ type: 'speech_started' });
stt.publish({ type: 'partial_transcript', text: 'different question', isFinal: false });
stt.publish({ type: 'partial_transcript', text: 'different', isFinal: false });
await new Promise((resolve) => setTimeout(resolve, 400));
assert.equal(orchestrator.controller.state, 'thinking', 'A revised partial below the word threshold must replace the old interruption candidate');
